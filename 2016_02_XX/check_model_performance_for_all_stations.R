...
if (station_type == "calibration") {type_folder = "analysis_iwmi/calibration/"}
if (station_type == "validation" ) {type_folder = "analysis_iwmi/validation/"}
...
chart_for_degree_day_factor   <- chart_for_degree_day_factor   + annotate("text", x = 2.00, y = river_number, label = substr(river_name[1], 1, 7), size = 2.00)
chart_for_min_soil_depth_frac <- chart_for_min_soil_depth_frac + annotate("text", x = 2.00, y = river_number, label = substr(river_name[1], 1, 7), size = 2.00)
chart_for_log_ksat            <- chart_for_log_ksat            + annotate("text", x = 0.75, y = river_number, label = substr(river_name[1], 1, 7), size = 2.00)
chart_for_stor_cap            <- chart_for_stor_cap            + annotate("text", x = 1.50, y = river_number, label = substr(river_name[1], 1, 7), size = 2.00)
chart_for_log_recession_coef  <- chart_for_log_recession_coef  + annotate("text", x = 1.50, y = river_number, label = substr(river_name[1], 1, 7), size = 2.00)

}

# finalizing the plot
chart_for_degree_day_factor   <- chart_for_degree_day_factor   + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) + scale_size_continuous(limits = c(0.001, 1.0), breaks = seq(0,1,0.05)) + ylab("") + scale_x_continuous(breaks = seq( 0.50, 2.00, 0.50)) + scale_y_continuous(breaks = seq(0, length(river), 1), limits = c(0, length(river) + 1), expand = c(0,0))
chart_for_min_soil_depth_frac <- chart_for_min_soil_depth_frac + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) + scale_size_continuous(limits = c(0.001, 1.0), breaks = seq(0,1,0.05)) + ylab("") + scale_x_continuous(breaks = seq( 0.50, 2.00, 0.50)) + scale_y_continuous(breaks = seq(0, length(river), 1), limits = c(0, length(river) + 1), expand = c(0,0))
chart_for_log_ksat            <- chart_for_log_ksat            + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) + scale_size_continuous(limits = c(0.001, 1.0), breaks = seq(0,1,0.05)) + ylab("") + scale_x_continuous(breaks = seq(-0.50, 1.75, 0.25)) + scale_y_continuous(breaks = seq(0, length(river), 1), limits = c(0, length(river) + 1), expand = c(0,0))
chart_for_stor_cap            <- chart_for_stor_cap            + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) + scale_size_continuous(limits = c(0.001, 1.0), breaks = seq(0,1,0.05)) + ylab("") + scale_x_continuous(breaks = seq( 0.75, 1.50, 0.25)) + scale_y_continuous(breaks = seq(0, length(river), 1), limits = c(0, length(river) + 1), expand = c(0,0))
chart_for_log_recession_coef  <- chart_for_log_recession_coef  + theme(axis.ticks.y = element_blank(), axis.text.y = element_blank()) + scale_size_continuous(limits = c(0.001, 1.0), breaks = seq(0,1,0.05)) + ylab("") + scale_x_continuous(breaks = seq(-1.00, 1.50, 0.50)) + scale_y_continuous(breaks = seq(0, length(river), 1), limits = c(0, length(river) + 1), expand = c(0,0))
